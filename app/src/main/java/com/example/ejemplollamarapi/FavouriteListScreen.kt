...
@Composable
fun FavouriteListScreen(favouriteProductViewModel: FavouriteProductViewModel, context: Context) {
    val favoriteList = favouriteProductViewModel.getFavoriteProductList().collectAsState(initial = emptyList())
    val convertedList: List<ProductResponse> = favoriteList.value.map { productToProductResponse(it) }
    val isLoading: Boolean by favouriteProductViewModel.isLoading.observeAsState(initial = false)
    if (isLoading) {
        LoadingScreen()
    } else {
        FavoriteListView(convertedList, favouriteProductViewModel, context)
    }
}
                    ...
                    Row(
                        ...
                        FilledIconButton(
                            ...
                            onClick = {
                                ...
                                favouriteProductViewModel.deleteFavoriteProduct(product.id)
                            }
                        ) {
                            Icon(
